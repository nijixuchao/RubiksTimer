...
BOOL sq1phase2(int edge, int corner, BOOL topEdgeFirst, BOOL botEdgeFirst, int ml, int maxl, int depth, int lm) {
    ...
    if(lm!=0 && topEdgeFirst == botEdgeFirst) {
        int edgex = [SQSquare TwistMove][edge];
        int cornerx = [SQSquare TwistMove][corner];
        
        if ([SQSquare SquarePrun][edgex<<1|(1-ml)] < maxl && [SQSquare SquarePrun][cornerx<<1|(1-ml)] < maxl) {
            move[depth] = 0;
    ...
    if (lm <= 0){
        ...
        int edgex = topEdgeFirstx ? [SQSquare TopMove][edge] : edge;
        int cornerx = topEdgeFirstx ? corner : [SQSquare TopMove][corner];
        int m = topEdgeFirstx ? 1 : 2;
        int prun1 = [SQSquare SquarePrun][edgex<<1|ml];
        int prun2 = [SQSquare SquarePrun][cornerx<<1|ml];
        while (m < 12 && prun1 <= maxl && prun1 <= maxl) {
            ...
            if (topEdgeFirstx) {
                edgex = [SQSquare TopMove][edgex];
                prun1 = [SQSquare SquarePrun][edgex<<1|ml];
                m += 1;
            } else {
                cornerx = [SQSquare TopMove][cornerx];
                prun2 = [SQSquare SquarePrun][cornerx<<1|ml];
                m += 2;
            }
        }
    }
    ...
    if (lm <= 1){
        ...
        int edgex = botEdgeFirstx ? [SQSquare BottomMove][edge] : edge;
        int cornerx = botEdgeFirstx ? corner : [SQSquare BottomMove][corner];
        ...
        while (m < (maxl > 6 ? 6 : 12) && prun1 <= maxl && prun1 <= maxl) {
            ...
            if (botEdgeFirstx) {
                edgex = [SQSquare BottomMove][edgex];
                prun1 = [SQSquare SquarePrun][edgex<<1|ml];
                m += 1;
            } else {
                cornerx = [SQSquare BottomMove][cornerx];
                prun2 = [SQSquare SquarePrun][cornerx<<1|ml];
                m += 2;
            }
        }

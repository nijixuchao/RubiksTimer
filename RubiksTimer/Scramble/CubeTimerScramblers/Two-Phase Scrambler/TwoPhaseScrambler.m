...
NSString *const filePath = @"cached3x3";
...
-(NSString*)scramble {
    if (cachedScrambles.count > 0) {
        ...
        NSString *scramble = blockIsExecuting ? @"No scramble available" : [TwoPhaseScrambler generateScramble];
        [self cacheScramblesIfIdle];
        return scramble;
    }
}
...
+(NSString*) generateRandomCube {
    NSMutableArray *edges = [NSMutableArray arrayWithObjects:@"UF", @"UR", @"RD", @"RB", @"LU", @"LF", @"DB", @"DL", @"FR", @"UB", @"DF", @"BL", nil];
    NSMutableArray *corners = [NSMutableArray arrayWithObjects:@"UFR", @"RFD", @"RDB", @"RBU", @"LFU", @"LUB", @"DLB", @"LDF", nil];
    ...
    while (edges.count > 0) {
        ...
        int numToMove = randomnumber(2); //Won't work on lower than 4.3
        for (int x = 0; x < numToMove; x++) {
...
+(NSString*)generateScramble {
    ...
    while (!validCube) {
        ...
        solvedCube = solutionForFacelets(edgesAndCornersToSequential((char*)[randomCube UTF8String]), 21, 1000, 0, mask);
